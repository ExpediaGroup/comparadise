...
import {
  runManifestCompare,
  type RunManifestCompareDeps
} from '../src/run-manifest-compare';
import type { CompareResult } from '../src/manifest-compare';
...
function makeDeps(
  overrides: Partial<RunManifestCompareDeps> = {}
): RunManifestCompareDeps {
  return {
...
describe('runManifestCompare', () => {
  ...
  describe('outcome: match', () => {
    it('sets a success commit status', async () => {
      ...
      await runManifestCompare(params, makeDeps());
