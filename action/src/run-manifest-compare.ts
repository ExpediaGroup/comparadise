...
export interface RunManifestCompareDeps {
  classify: (params: CompareParams) => Promise<CompareResult>;
...
export interface RunManifestCompareParams {
  bucket: string;
  prSha: string;
...
export async function runManifestCompare(
  params: RunManifestCompareParams,
  deps: RunManifestCompareDeps
): Promise<void> {
